...
$\lst^{(1{:}k)}=\lirr+(1-\lirr)\rho^{\,k}\to\lirr$ (Part~I; the exogenous-family product
of~\cite{matijasevic2026gg}), so the floor on $a_B$---and hence on
...
$\Sigma_F=\{\text{bad }c:\ c\sim_F c'\text{ for some good }c'\}$ is inherited by every gate built within
$F$ (its GG1), so a system generating its own gates has an escape floor fixed at $\lambda_F=\mu(\Sigma_F)$,
invariant to generation depth, gates-checking-gates, and prompt diversity (its GG2). Two moves lower it:
importing an \emph{exogenous} family multiplies independent blind spots
($\mu(\Sigma_F\cap\Sigma_N)=\lambda_F\lambda_N$, driving $\lred$ down), and a \emph{reality-grounded}
gate---one deciding by ground truth rather than any family's representation---removes the grounded part
of $\lirr$. We take this floor, and its two levers, as given, and use them below.
...
(Part~I; the exogenous-family product of~\cite{matijasevic2026gg}). The posterior map $a_B\mapsto\fesc$~\eqref{eq:fdef} is a
