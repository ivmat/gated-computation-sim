...
distribution and $\lambda_F=\mu(\Sigma_F)$---is accepted by every \emph{sound} gate built within $F$
(its GG1; and, under GG's uniform likelihood-ratio bound, by any gate with false-reject mass $\rho_0$
except on a stealth set of mass at most $\kappa\rho_0$, vanishing as $\rho_0\to0$---absent a finite
$\kappa$, only the sound case). Consequently, on the sound-gate case, within-family generation cannot
drive the escape floor below $\lambda_F$---however deep the generation, however many gates check gates,
however diverse the prompts (its GG2; we use only this lower-bound direction, which is what our
...
irreducible floor is a constant $q_{\mathrm{irr}}>0$, which we take as exogenous to a fixed specification
and executor (its epistemic value is fixed in Assumption~\ref{ass:gate}; on the scaling route, a
genuinely better executor lowers $\pi$ and hence $\fesc$ separately---a different lever, out of scope
here). We compare the cost of
...
$a_B(n)=\lirr+(a_{B,0}-\lirr)\,c^{\,n}$ with $a_{B,0}>\lirr$ and per-family multiplier $c\in(0,1)$
(Part~I's decomposition with conditional independence across families; \S\ref{sec:escape}). We assume the good-acceptance rate is
held \emph{constant}, $a_G(n)\equiv a_G>0$ (a calibrated aggregator, not raw conjunction), with
$0<\pi<1$, and set the irreducible floor in the escape metric to
$q_{\mathrm{irr}}=\pi\lirr/(\pi\lirr+(1-\pi)a_G)$. Then $a_B\mapsto\fesc$~\eqref{eq:fdef} is a
\emph{fixed} M\"obius transform, increasing with derivative bounded away from $0$ and $\infty$ on
$[\lirr,a_{B,0}]$, so the geometric decay of $a_B(n)-\lirr$ carries to the induced escape at the same
rate:
\begin{equation}\label{eq:gaterate}
f_{\mathrm{gate}}(n)-q_{\mathrm{irr}}\;=\;\Theta(c^{\,n}),
\end{equation}
which is all Proposition~\ref{prop:cost} uses. Each family costs a fixed $g_F$. The constancy of $a_G$
is load-bearing: a varying $a_G(n)$---in particular $a_G(n)=\prod_i a_{G,i}\to0$ under pure unanimous
stacking---breaks the rate, and for $\lirr>0$ drives $\fesc\to1$ rather than $q_{\mathrm{irr}}$; holding
$a_G$ fixed is exactly what a calibrated aggregator, not raw conjunction, buys.
\end{assumption}
...
\emph{Gate.} By~\eqref{eq:gaterate} there are constants $0<A\le B$ with
$A c^n\le f_{\mathrm{gate}}(n)-q_{\mathrm{irr}}\le B c^n$, so $f_{\mathrm{gate}}(n)\le q_{\mathrm{irr}}+\delta$
holds once $B c^n\le\delta$, giving a family count
\begin{equation}\label{eq:gate}
n_{\mathrm{gate}}(\delta)=\Big\lceil\tfrac{\ln(B/\delta)}{\ln(1/c)}\Big\rceil=O\!\big(\log(1/\delta)\big),
\end{equation}
logarithmic in $1/\delta$, at total cost $g_F\,n_{\mathrm{gate}}(\delta)$. (The representative closed
form $f_{\mathrm{gate}}(n)=q_{\mathrm{irr}}+(\fesc_0-q_{\mathrm{irr}})c^{\,n}$, with the same rate, is used
only for the illustration in Table~\ref{tab:sep}.)
...
\end{proposition}
\begin{proof}
Put $x=1/\delta\to\infty$. Then $C=\kappa^{1/p}x^{1/p}$ and, by~\eqref{eq:gate},
$g_F\,n_{\mathrm{gate}}\le g_F\big(1+\ln(Bx)/\ln(1/c)\big)$ with $B$ the upper $\Theta$-constant
of~\eqref{eq:gaterate}, so the ratio is $O(\log x)/x^{1/p}\to0$, since $\ln x=o(x^{1/p})$ for every $p>0$.
\end{proof}
...
from the closed forms in the caption.
